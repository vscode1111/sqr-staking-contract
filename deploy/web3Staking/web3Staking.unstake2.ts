...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment): Promise<void> => {
  await callWithTimerHre(async () => {
    ...
    const { user2Address, user1ERC20Token, user2WEB3Staking } = context;

    const decimals = Number(await user1ERC20Token.decimals());

    const currentAllowance = await user1ERC20Token.allowance(user2Address, web3StakingAddress);
    ...
    await waitTx(user2WEB3Staking.unstake(params.userStakeId), 'unstake');
  }, hre);
};

func.tags = [`${WEB3_STAKING_NAME}:unstake2`];
