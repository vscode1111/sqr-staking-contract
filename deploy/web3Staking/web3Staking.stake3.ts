...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment): Promise<void> => {
  await callWithTimerHre(async () => {
    ...
    const { user3Address, user3ERC20Token, user3WEB3Staking } = context;

    const decimals = Number(await user3ERC20Token.decimals());

    const currentAllowance = await user3ERC20Token.allowance(user3Address, web3StakingAddress);
    ...
    if (params.amount > currentAllowance) {
      ...
      await waitTx(user3ERC20Token.approve(web3StakingAddress, askAllowance), 'approve');
      console.log(
    ...
    await waitTx(user3WEB3Staking.stake(params.amount), 'stake');
  }, hre);
};

func.tags = [`${WEB3_STAKING_NAME}:stake3`];
