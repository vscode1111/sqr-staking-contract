...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment): Promise<void> => {
  await callWithTimerHre(async () => {
    ...
    const { user2Address, user1ERC20Token, user2SQRStaking } = context;

    const decimals = Number(await user1ERC20Token.decimals());

    const currentAllowance = await user1ERC20Token.allowance(user2Address, sqrStakingAddress);
    ...
    await waitTx(user2SQRStaking.unstake(params.userStakeId), 'unstake');
  }, hre);
};

func.tags = [`${SQR_STAKING_NAME}:unstake2`];
