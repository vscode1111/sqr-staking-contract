...
import { TokenConfig, getTokenArgs, tokenConfig } from '~seeds';
...
export async function getERC20TokenContext(
  ...
  if (typeof deployData === 'string') {
    ...
    ownerERC20Token = await testERC20TokenFactory
      .connect(owner)
      .deploy(...getTokenArgs({ ...tokenConfig, ...deployData } as TokenConfig));
  }
  ...
  const companyERC20Token = ownerERC20Token.connect(company);
